...
int main(int argc, char** argv) {
  ...
  // *env << "Start trace! Nr: " << destinationAddress.s_addr  << "\n";
  ...
  *env << "First port Nr: " << htons(rtpPort.num())  << "\n";
  const Port rtcpPort(rtcpPortNum);
  *env << "Secon port Nr: " << htons(rtcpPort.num())  << "\n";
  ...
  *env << "Play this stream using the URL \"" << url << "\"\n";
  delete[] url;
  //pump(env, scheduler, rtspServer, 20000, 100);
