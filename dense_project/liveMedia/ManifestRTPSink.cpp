...
void ManifestRTPSink::sendPacketIfNecessary() {
  fprintf(stderr, "\n     sendPacketIfNecessary() -> %u\n", fPacketCount);
  if (fNumFramesUsedSoFar > 0) {
    // Send the packet:
//TEST_LOSS
    if (fPacketCount % 10000000 != 0) // simulate packet loss #####
...
void ManifestRTPSink::setXBit() {
  unsigned rtpHdr = fOutBuf->extractWord(0);
  rtpHdr |= 0x40000000;
  fOutBuf->insertWord(rtpHdr, 0);
}

unsigned ManifestRTPSink::frameSpecificHeaderSize() const {
...
Boolean ManifestRTPSink::continuePlaying() {
  fprintf(stderr, "\n     ManifestRTPSink::continuePlaying()\n");
  // Send the first packet.
  // (This will also schedule any future sends.)
  buildAndSendPacket(True);
  return True;
...
void ManifestRTPSink::buildAndSendPacket(Boolean isFirstPacket) {
  ...
  // RTP header:
  fSpecialHeaderPosition = fOutBuf->curPacketSize(); //Where is the special header going? Right after the SSRC
  fSpecialHeaderSize = specialHeaderSize(); //How biug is the header? Right now: 4bytes 
    ...
    int he = checkSource->getCurChunk(); 
    fprintf(stderr, "The checkSource->curChunk: %d\n\n", he);


  unsigned short chunk_reference = (unsigned short)he;
  unsigned short length_reference = 0x0000;
  unsigned videoSpecificHeader = (chunk_reference<<16);
  videoSpecificHeader |= (length_reference);

  fprintf(stderr, "The SSRC: %u\nHeader the header: %u chunk reference: %hu\nlength reference: %hu\n", SSRC(), videoSpecificHeader, videoSpecificHeader>>16, videoSpecificHeader);
  fOutBuf->enqueueWord(videoSpecificHeader);
  u_int32_t yuo = fOutBuf->extractWord(fSpecialHeaderPosition);
  fprintf(stderr, "ManifestRTPSink videoSpecificHeader DOUBLE CHECK: %u chunk reference: %hu\nlength reference: %hu\n", yuo, yuo>>16, yuo);
...
Boolean ManifestRTPSink::
frameCanAppearAfterPacketStart(unsigned char const* /*frameStart*/,
			       unsigned /*numBytesInFrame*/) const {
  return fAllowMultipleFramesPerPacket;
}

char const* ManifestRTPSink::sdpMediaType() const {
