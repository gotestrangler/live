...
char* ServerMediaSession::generateSDPDescription() {
  ...
  if (fIsSSM) {
    //fprintf(stderr, "       generateSDPDescription() -> IS SSM\n");
  ...
  do {
    ...
    if (sdpLength == 0){
      //fprintf(stderr, "       generateSDPDescription() -> sdpLength == 0, the session has no usable subsessions - BREAK\n");
      break; // the session has no usable subsessions
    }

    //fprintf(stderr, "       generateSDPDescription() -> sdpLines = %s, subteller: %d\n", sdpLength, subTeller);
  
    // Unless subsessions have differing durations, we also have a "a=range:" line:
    //fprintf(stderr, "       generateSDPDescription() -> call to duration()\n");
    float dur = duration();
    if (dur == 0.0) {
    ...
    //fprintf(stderr, "       generateSDPDescription() -> building sdpPrefixFmt: \n %s \n", sdpPrefixFmt);
    ...
    //fprintf(stderr, "       generateSDPDescription() -> Generate the SDP prefix (session-level lines)\n");
