...
::rtspURL(ServerMediaSession const* serverMediaSession, int clientSocket) const {
  ...
  //fprintf(stderr, "rtspURL - socket: %s\n", urlPrefix);
...
char* RTSPServer::rtspURLPrefix(int clientSocket) const {
  //fprintf(stderr, "rtspURLPrefix - int socket: %d\n", clientSocket);
...
RTSPServer::RTSPServer(UsageEnvironment& env, int ourSocket, Port ourPort, UserAuthenticationDatabase* authDatabase, unsigned reclamationSeconds)
...
::handleCmd_SETUP(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  fprintf(stderr, "Hello, i have arrived at handleCmd_SETUP. SetupCnt: %d\n", setup_cnt++);

  if(setup_cnt){
    fprintf(stderr, "     This is not the first SETUP, so lets try!\n");
    /*
    Trenger: 
    env = envir() 
    inputFileName = 
    rtpPortNum = choose
    RTPSink* videoSink = lager
    rtspServer = fOurServer
    
    For play(): 
    hvordan i helvette kommer den til eventloop uansett? Den play loopen er jo evig? 
  ...
  do {
    ...
    if (sms == NULL) {
      fprintf(stderr, "   handleCmd_SETUP Server Media Session = NULL first time\n");
  ...
  fprintf(stderr, "   handleCmd_SETUP Server Media Session = NULL for real this time - notFound()\n");
  ...
  fprintf(stderr, "   handleCmd_SETUP Server Media Session = NULL\n");
