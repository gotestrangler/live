...
///////// Mine greier /////////// 
  ...
  void convertIGMP(unsigned char* buffer, igmpv3_report *igmp)
{
    igmp->igmp_type = buffer[0];
    igmp->igmp_resv1 = buffer[1];
    igmp->igmp_cksum = (buffer[2] <<  8) | buffer[3];
    igmp->igmp_resv2 = (buffer[4] <<  8) | buffer[5];
    igmp->igmp_ngrec = (buffer[6] <<  8) | buffer[7];
...
::createNewDestRecord(struct in_addr const& addr, Port const& port, u_int8_t ttl,
  ...
  fprintf(stderr, "     createNewDestRecord: %d\n", port.num());
...
Groupsock::changeDestinationParameters(struct in_addr const& newDestAddr,
  ...
  fprintf(stderr, "     changeDestinationParameters: %d\n", newDestPort.num());
