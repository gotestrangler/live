...
void* BasicHashTable::Add(char const* key, void* value) {
  ...
  if (entry != NULL) {
    ...
    //fprintf(stderr, "Add -> making entry\n");
...
Boolean BasicHashTable::Remove(char const* key) {
  unsigned index;
  //fprintf(stderr, "Remove -> destroying entry: %s\n", key);
  TableEntry* entry = lookupKey(key, index);
...
void* BasicHashTable::Lookup(char const* key) const {
  ...
  TableEntry* entry = lookupKey(key, index);
  if (entry == NULL){
    //fprintf(stderr, "Lookup entry er null -> gå videre til lookupKey: %s\n", key);
